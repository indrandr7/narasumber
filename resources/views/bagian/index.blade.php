...
<div class="content-wrapper">
                            ...
                            <table id="tabeldata" class="table table-bordered table-hover table-striped" width="100%">
                                <thead>
                                    <tr>
                                        ...
                                        <th class="ratatengah warnabg-gray vertikaltengah" width="25%">Nama Bagian</th>
                                        <th class="ratatengah warnabg-gray vertikaltengah" width="25%">Biro</th>
                                        <th class="ratatengah warnabg-gray vertikaltengah" width="20%">Pejabat</th>
                                        <th class="ratatengah warnabg-gray vertikaltengah" width="15%">NIP</th>
  ...
  <script type="text/javascript">
    ...
    function showFormAdd(){
        $('#modalku').modal('show').find('#modalku_content').load("{{ url('/bagian/add') }}");
    }

    function showFormEdit(id){
        $('#modalku').modal('show').find('#modalku_content').load("{{ url('/bagian/edit') }}?id="+id);
    }
    ...
    $(function (){
        ...
        var table = $('#tabeldata').DataTable({
            processing: true,
            serverSide: true,
            ajax: "{{ url('/bagian/getlist') }}",
            columns: [
                {data: 'DT_RowIndex', name: 'DT_RowIndex'},
                {data: 'namabagian', name: 'namabagian'},
                {data: 'biro', name: 'biro'},
                {data: 'namapejabat', name: 'namapejabat'},
                {data: 'nip', name: 'nip'},
                {data: 'action'}
            ],
            columnDefs: [
