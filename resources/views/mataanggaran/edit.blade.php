    ...
    <form class="form-horizontal" id="formulir" name="formulir" method="post" enctype="multipart/form-data" novalidate="novalidate">
        ...
        <div class="card-body">
            ...
            <div class="col-sm-9">
              <select class="form-control" id="id_bagian" name="id_bagian">
                @foreach ($bagian as $bg)
                  @if ($bg->id_bagian == $mak->id_bagian)
                  <option value="{{ $bg->id_bagian }}" selected>{{ $bg->nama_bagian }}</option>
                  @else
          ...
          <div class="form-group row">
            ...
            <div class="col-sm-9">
              <input type="text" class="form-control" id="kodemak" name="kodemak" value="{{ $mak->kodemak }}">
              <input type="hidden" class="form-control" id="id_mataanggaran" name="id_mataanggaran" value="{{ $mak->id_mak }}">
            </div>
          </div>
          <div class="form-group row">
            <label for="mataanggaran" class="col-sm-3 col-form-label">Nama Kegiatan</label>
            <div class="col-sm-9">
              <input type="input" class="form-control" id="namakegiatan" name="namakegiatan" value="{{ $mak->namakegiatan }}">
            </div>
          </div>
          <div class="form-group row">
            <label for="mataanggaran" class="col-sm-3 col-form-label">Tahun</label>
            <div class="col-sm-2">
              <input type="input" class="form-control" id="tahun" name="tahun" value="{{ $mak->tahun }}" number>
            </div>
          </div>
        </div>
  ...
  <script>
    ...
    $.validator.setDefaults({
      submitHandler: function(){
        ...
        $.ajax({
            url: "{{ url('/mataanggaran/saveupdate') }}",
            type: 'POST',
